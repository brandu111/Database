...
export const api = {
  ...
  syncAuRenewals: (offset: number, limit = 8) => req<{ processed: number; diffs: { id: string; name: string; number: string; stored: string; official: string }[]; errors: { name: string; error: string }[]; offset: number; total: number }>('POST', '/api/marks/sync-au-renewals', { offset, limit }),
  applyAuRenewals: (updates: { id: string; date: string }[]) => req<{ applied: number; changed: { id: string; name: string; before: string; after: string }[] }>('POST', '/api/marks/apply-au-renewals', { updates }),
  ...
  fetchAuLogos: (offset: number, limit = 12, overwrite = false) => req<{ processed: number; updated: number; withImageUrl: number; noImageOnRegister: number; downloadFailed: number; noNumber: number; notFound: number; rateLimited: number; authErr: number; otherErr: number; alreadyHave: number; offset: number; total: number; errors: { name: string; error: string }[] }>('POST', '/api/marks/logos/fetch-au', { offset, limit, overwrite }),
